...
const ticketType = 'hoisting_work';
const extTable = 'hoisting_tickets';
const prefix = 'HT';

export const getHoistingWorkList = async (req: Request, res: Response, next: NextFunction) => {
  let conn: any = null;
  try {
    ...
    let sql = `SELECT wp.id, wp.ticket_no, wp.status as main_status, wp.applicant_id, wp.created_at,
      ...
      ext.hoisting_weight, ext.equipment_name
...
export const getHoistingWorkDetail = async (req: Request, res: Response, next: NextFunction) => {
  let conn: any = null;
  try {
...
export const createHoistingWork = async (req: Request, res: Response, next: NextFunction) => {
  let conn: any = null;
  try {
    const { projectName, workLocation, workContent, startTime, endTime, hoistingWeight, hoistingHeight, hoistingRadius, equipmentName, equipmentModel, hasGroundSupport, safetyMeasures } = req.body;
    const userId = (req as any).user?.userId;
    const canWork = await checkCanWork(userId, 'hoisting');
    ...
    try {
      ...
      await conn.execute(`INSERT INTO ${extTable} (permit_id, hoisting_weight, hoisting_height, hoisting_radius, equipment_name, equipment_model, has_ground_support, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`, [ticketId, hoistingWeight, hoistingHeight, hoistingRadius, equipmentName, equipmentModel, hasGroundSupport?1:0]);
      await conn.commit();
      await recordTrace({entity_type:'work_ticket',entity_id:ticketId,entity_no:ticketNo,action:'create',action_label:'创建吊装作业',operator_id:userId,operator_name:(req as any).user?.realName||'',snapshot_after:{status:'1',ticketNo,projectName,hoistingWeight}});
...
export const updateHoistingWork = async (req: Request, res: Response, next: NextFunction) => {
  let conn: any = null;
  try {
    const { id } = req.params; const { projectName, workLocation, workContent, startTime, endTime, hoistingWeight, hoistingHeight, hoistingRadius, equipmentName, equipmentModel, hasGroundSupport, safetyMeasures } = req.body;
    ...
    try {
      ...
      await conn.execute(`UPDATE ${extTable} SET hoisting_weight=?, hoisting_height=?, hoisting_radius=?, equipment_name=?, equipment_model=?, has_ground_support=?, updated_at=NOW() WHERE permit_id=?`, [hoistingWeight, hoistingHeight, hoistingRadius, equipmentName, equipmentModel, hasGroundSupport?1:0, id]);
...
export const submitHoistingWork = async (req: Request, res: Response, next: NextFunction) => {
  let conn: any = null;
  try {
...
export const approveHoistingWork = async (req: Request, res: Response, next: NextFunction) => {
  let conn: any = null;
  try {
