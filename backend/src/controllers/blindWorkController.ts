...
const ticketType = 'blind_work';
const extTable = 'blind_tickets';
const prefix = 'BD';

export const getBlindWorkList = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const getBlindWorkDetail = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const createBlindWork = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { projectName, workLocation, workContent, startTime, endTime, pipelineName, pipelineSpec, medium, temperature, pressure, blindSpec, safetyMeasures } = req.body;
    const userId = (req as any).user?.userId;
    const canWork = await checkCanWork(userId, 'blind');
    ...
    try {
      ...
      await conn.execute(`INSERT INTO ${extTable} (permit_id, pipeline_name, pipeline_spec, medium, temperature, pressure, blind_spec, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`, [ticketId, pipelineName, pipelineSpec, medium, temperature, pressure, blindSpec]);
      await conn.commit();
      await recordTrace({entity_type:'work_ticket',entity_id:ticketId,entity_no:ticketNo,action:'create',action_label:'创建盲板抽堵作业',operator_id:userId,operator_name:(req as any).user?.realName||'',snapshot_after:{status:'1',ticketNo,projectName,pipelineName}});
      res.status(201).json({ code: 200, msg: 'success', data: { id: ticketId, ticketNo } });
    } catch (err) { await conn.rollback(); throw err; }
  } catch (error) { next(error); }
};

export const updateBlindWork = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params; const { projectName, workLocation, workContent, startTime, endTime, pipelineName, pipelineSpec, medium, temperature, pressure, blindSpec, safetyMeasures } = req.body;
    ...
    try {
      ...
      await conn.execute(`UPDATE ${extTable} SET pipeline_name=?, pipeline_spec=?, medium=?, temperature=?, pressure=?, blind_spec=?, updated_at=NOW() WHERE permit_id=?`, [pipelineName, pipelineSpec, medium, temperature, pressure, blindSpec, id]);
      await conn.commit(); res.json({ code: 200, msg: 'success' });
    } catch (err) { await conn.rollback(); throw err; }
  } catch (error) { next(error); }
};

export const submitBlindWork = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const approveBlindWork = async (req: Request, res: Response, next: NextFunction) => {
  try {
