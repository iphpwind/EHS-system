...
export const getConfinedSpaceList = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    let sql = `SELECT wp.id, wp.ticket_no, wp.status as main_status, wp.applicant_id, wp.created_at,
      ...
      LEFT JOIN ${extTable} ext ON wp.id = ext.ticket_id
...
export const getConfinedSpaceDetail = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const [extRows] = await conn.execute<RowDataPacket[]>(`SELECT * FROM ${extTable} WHERE ticket_id = ?`, [id]);
...
export const createConfinedSpace = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    try {
      ...
      await conn.execute(`INSERT INTO ${extTable} (ticket_id, space_name, space_type, oxygen_content, toxic_content, ventilation_method, power_isolation, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`, [ticketId, spaceName, spaceType, oxygenContent, toxicContent, ventilationMethod, powerIsolation?1:0]);
      await conn.commit();
...
export const updateConfinedSpace = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    try {
      ...
      await conn.execute(`UPDATE ${extTable} SET space_name=?, space_type=?, oxygen_content=?, toxic_content=?, ventilation_method=?, power_isolation=?, updated_at=NOW() WHERE ticket_id=?`, [spaceName, spaceType, oxygenContent, toxicContent, ventilationMethod, powerIsolation?1:0, id]);
...
export { startWork, finishWork, closeWork, guardianSignIn, guardianConfirmEnd } from '../controllers/highWorkController';
