...
export const deleteSupply = async (req: Request, res: Response, next: NextFunction) => {
  try { const conn = await getConnection(); await conn.execute('DELETE FROM emergency_supplies WHERE id = ?', [req.params.id]); res.json({ success: true }); }
  catch (e) { next(e); }
};
...
export const getLawTypes = async (req: Request, res: Response, next: NextFunction) => {
  try { const conn = await getConnection(); const [rows] = await conn.execute<RowDataPacket[]>('SELECT * FROM law_types WHERE status = 1 ORDER BY sort_order'); res.json({ success: true, data: rows }); }
  catch (e) { next(e); }
};
