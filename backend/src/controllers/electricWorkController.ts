...
const ticketType = 'electric_work';
const extTable = 'electricwork_tickets';
const prefix = 'EL';
...
export const createElectricWork = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    try {
      ...
      await conn.execute(`INSERT INTO ${extTable} (permit_id, voltage_level, power_capacity, wiring_method, has_ground_protection, has_leakage_protection, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())`, [ticketId, voltageLevel, powerCapacity, wiringMethod, hasGroundProtection?1:0, hasLeakageProtection?1:0]);
      await conn.commit();
      await recordTrace({entity_type:'work_ticket',entity_id:ticketId,entity_no:ticketNo,action:'create',action_label:'创建临时用电作业',operator_id:userId,operator_name:(req as any).user?.realName||'',snapshot_after:{status:'1',ticketNo,projectName,voltageLevel}});
...
export const updateElectricWork = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    try {
      ...
      await conn.execute(`UPDATE ${extTable} SET voltage_level=?, power_capacity=?, wiring_method=?, has_ground_protection=?, has_leakage_protection=?, updated_at=NOW() WHERE permit_id=?`, [voltageLevel, powerCapacity, wiringMethod, hasGroundProtection?1:0, hasLeakageProtection?1:0, id]);
