...
const ticketType = 'earth_work';
const extTable = 'earth_tickets';
const prefix = 'EW';

export const getEarthWorkList = async (req: Request, res: Response, next: NextFunction) => {
  let conn: any = null;
  try {
    ...
    let sql = `SELECT wp.id, wp.ticket_no, wp.status as main_status, wp.applicant_id, wp.created_at,
      ...
      ext.excavation_depth, ext.support_method
...
export const getEarthWorkDetail = async (req: Request, res: Response, next: NextFunction) => {
  let conn: any = null;
  try {
...
export const createEarthWork = async (req: Request, res: Response, next: NextFunction) => {
  let conn: any = null;
  try {
    const { projectName, workLocation, workContent, startTime, endTime, excavationDepth, excavationArea, hasUndergroundFacilities, facilitiesDesc, supportMethod, safetyMeasures } = req.body;
    const userId = (req as any).user?.userId;
    const canWork = await checkCanWork(userId, 'earth');
    ...
    try {
      ...
      await conn.execute(`INSERT INTO ${extTable} (permit_id, excavation_depth, excavation_area, has_underground_facilities, facilities_desc, support_method, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())`, [ticketId, excavationDepth, excavationArea, hasUndergroundFacilities?1:0, facilitiesDesc, supportMethod]);
      await conn.commit();
      await recordTrace({entity_type:'work_ticket',entity_id:ticketId,entity_no:ticketNo,action:'create',action_label:'创建动土作业',operator_id:userId,operator_name:(req as any).user?.realName||'',snapshot_after:{status:'1',ticketNo,projectName,excavationDepth}});
...
export const updateEarthWork = async (req: Request, res: Response, next: NextFunction) => {
  let conn: any = null;
  try {
    const { id } = req.params; const { projectName, workLocation, workContent, startTime, endTime, excavationDepth, excavationArea, hasUndergroundFacilities, facilitiesDesc, supportMethod, safetyMeasures } = req.body;
    ...
    try {
      ...
      await conn.execute(`UPDATE ${extTable} SET excavation_depth=?, excavation_area=?, has_underground_facilities=?, facilities_desc=?, support_method=?, updated_at=NOW() WHERE permit_id=?`, [excavationDepth, excavationArea, hasUndergroundFacilities?1:0, facilitiesDesc, supportMethod, id]);
...
export const submitEarthWork = async (req: Request, res: Response, next: NextFunction) => {
  let conn: any = null;
  try {
...
export const approveEarthWork = async (req: Request, res: Response, next: NextFunction) => {
  let conn: any = null;
  try {
