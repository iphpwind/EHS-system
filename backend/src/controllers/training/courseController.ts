...
export const getCourseList = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const [rows] = await conn.execute<RowDataPacket[]>(`SELECT * FROM training_courses ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`, [...params, Number(pageSize), offset]);
  ...
  try {
    ...
    if (req.body.sections && Array.isArray(req.body.sections)) {
      for (const sec of req.body.sections) {
        await conn.execute('INSERT INTO course_sections (course_id, section_no, title, duration, video_url, content_type, content_text) VALUES (?,?,?,?,?,?,?)',
          [result.insertId, sec.sectionNo || 0, sec.title, sec.duration || 0, sec.videoUrl || '', sec.contentType || 'video', sec.contentText || '']);
      }
    }
...
export const deleteCourse = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const conn = await getConnection();
    await conn.execute('DELETE FROM training_courses WHERE id = ?', [req.params.id]);
