...
const ticketType = 'broken_work';
const extTable = 'broken_tickets';
const prefix = 'BW';

export const getBrokenWorkList = async (req: Request, res: Response, next: NextFunction) => {
  let conn: any = null;
  try {
...
export const getBrokenWorkDetail = async (req: Request, res: Response, next: NextFunction) => {
  let conn: any = null;
  try {
...
export const createBrokenWork = async (req: Request, res: Response, next: NextFunction) => {
  let conn: any = null;
  try {
    const { projectName, workLocation, workContent, startTime, endTime, roadName, roadWidth, blockMethod, trafficGuide, safetyMeasures } = req.body;
    const userId = (req as any).user?.userId;
    const canWork = await checkCanWork(userId, 'broken');
    ...
    try {
      ...
      await conn.execute(`INSERT INTO ${extTable} (permit_id, road_name, road_width, block_method, traffic_guide, created_at, updated_at) VALUES (?, ?, ?, ?, ?, NOW(), NOW())`, [ticketId, roadName, roadWidth, blockMethod, trafficGuide]);
      await conn.commit();
      await recordTrace({entity_type:'work_ticket',entity_id:ticketId,entity_no:ticketNo,action:'create',action_label:'创建断路作业',operator_id:userId,operator_name:(req as any).user?.realName||'',snapshot_after:{status:'1',ticketNo,projectName,roadName}});
...
export const updateBrokenWork = async (req: Request, res: Response, next: NextFunction) => {
  let conn: any = null;
  try {
    const { id } = req.params; const { projectName, workLocation, workContent, startTime, endTime, roadName, roadWidth, blockMethod, trafficGuide, safetyMeasures } = req.body;
    ...
    try {
      ...
      await conn.execute(`UPDATE ${extTable} SET road_name=?, road_width=?, block_method=?, traffic_guide=?, updated_at=NOW() WHERE permit_id=?`, [roadName, roadWidth, blockMethod, trafficGuide, id]);
...
export const submitBrokenWork = async (req: Request, res: Response, next: NextFunction) => {
  let conn: any = null;
  try {
...
export const approveBrokenWork = async (req: Request, res: Response, next: NextFunction) => {
  let conn: any = null;
  try {
