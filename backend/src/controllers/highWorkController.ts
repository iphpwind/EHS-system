...
const ticketType = 'high_work';
const extTable = 'highwork_tickets';
const prefix = 'HW';
...
export const createHighWork = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    // 强制培训准入校验
    const canWork = await checkCanWork(userId, 'high_work');
    if (!canWork.allowed) {
    ...
    try {
      ...
      const [mainResult] = await conn.execute<OkPacket>(`INSERT INTO work_permits (ticket_no, ticket_type, ticket_level, project_name, work_location, work_content, status, applicant_id, apply_time, start_time, end_time, safety_measures, current_node, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, '1', ?, NOW(), ?, ?, ?, 0, NOW(), NOW())`, [ticketNo, ticketType, String(workLevel), projectName, workLocation, workContent, userId, startTime||null, endTime||null, safetyMeasures?JSON.stringify(safetyMeasures):null]);
      const ticketId = mainResult.insertId;
      await conn.execute(`INSERT INTO ${extTable} (permit_id, work_height, work_level, weather_condition, has_ground_protection, has_safety_net, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())`, [ticketId, workHeight, workLevel, weatherCondition, hasGroundProtection?1:0, hasSafetyNet?1:0]);
      await conn.commit();

      // 操作审计
      await recordTrace({
        entity_type: 'work_ticket', entity_id: ticketId, entity_no: ticketNo,
        action: 'create', action_label: `创建${statusTextMap[ticketType] || '作业票'}`,
        operator_id: userId, operator_name: (req as any).user?.realName || '',
        snapshot_after: { status: '1', ticketNo, projectName, workLevel }
      });
...
export const updateHighWork = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    await conn.beginTransaction();
    try {
      await conn.execute(`UPDATE work_permits SET project_name=?, work_location=?, work_content=?, start_time=?, end_time=?, ticket_level=?, safety_measures=?, updated_at=NOW() WHERE id=?`, [projectName, workLocation, workContent, startTime, endTime, String(workLevel), safetyMeasures?JSON.stringify(safetyMeasures):null, id]);
      await conn.execute(`UPDATE ${extTable} SET work_height=?, work_level=?, weather_condition=?, has_ground_protection=?, has_safety_net=?, updated_at=NOW() WHERE permit_id=?`, [workHeight, workLevel, weatherCondition, hasGroundProtection?1:0, hasSafetyNet?1:0, id]);
...
export const submitHighWork = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    await recordTrace({
      ...
      action: 'submit', action_label: '提交审批',
      operator_id: userId, operator_name: (req as any).user?.realName || '',
...
export const startWork = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    await conn.execute("UPDATE work_permits SET status = '6', actual_start_time = NOW(), updated_at = NOW() WHERE id = ?", [id]);

    // 操作审计
    await recordTrace({
      entity_type: 'work_ticket', entity_id: Number(id),
      action: 'start_work', action_label: '开始作业',
      operator_id: (req as any).user?.userId || 0, operator_name: (req as any).user?.realName || '',
...
export const finishWork = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    await conn.execute("UPDATE work_permits SET status = '7', actual_end_time = NOW(), updated_at = NOW() WHERE id = ?", [id]);
    ...
    await recordTrace({
      ...
      operator_id: op.userId || 0, operator_name: op.realName || '',
...
export const closeWork = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (s !== '7' && s !== '6') return res.status(400).json({ code: 400, msg: '当前状态不能关闭' });
    await conn.execute("UPDATE work_permits SET status = '8', updated_at = NOW() WHERE id = ?", [id]);
    await conn.execute(`INSERT INTO ticket_approvals (ticket_id, node_id, node_name, approver_id, approval_result, approval_opinion, approval_time) VALUES (?, 99, '关闭归档', ?, 'close', ?, NOW())`, [id, userId, comment]);
    ...
    await recordTrace({
      ...
      operator_id: userId, operator_name: (req as any).user?.realName || '',
      snapshot_before: { status: s }, snapshot_after: { status: '8' }
    });
