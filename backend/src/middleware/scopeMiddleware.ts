 ...
 * 注入到 req.scope 和 req.userDeptId，供各 Controller 过滤数据使用。
 *
 * Scope 级别：
 *   SELF  — 仅本人数据
 *   DEPT  — 本部门数据（默认）
  ...
  try {
    ...
    try {
      ...
      if (rows && rows.length > 0) {
        ...
        (req as any).userDeptId = user.department_id || 0;
        (req as any).scope = user.scope || 'DEPT'; // 默认部门级
      } else {
        (req as any).userDeptId = 0;
 ...
 * 
 * @param req    Express Request（需已通过 scopeMiddleware 注入 scope 和 userDeptId）
...
export function buildScopeWhere(
  ...
  deptIdField: string = 'department_id'
): { where: string; params: any[] } {
  const scope = (req as any).scope || 'DEPT';
  const userId = (req as any).user?.userId;
  const userDeptId = (req as any).userDeptId || 0;
