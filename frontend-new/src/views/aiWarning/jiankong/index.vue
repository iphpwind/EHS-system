<template>
  <div class="aijiankong">
    <div class="toptitle">
      <div style="position: fixed;color: #fff;margin: 33px;">设备状态：在线 启动时间：{{zjxx.updateTime}}</div>
    	<img src="@/assets/images/screenyxjs/title2.png" @load="topimageLoaded"/>
    </div>
			...
			<el-row :gutter="15">
				<el-col :span="14">
					<div class="mainleft">
						...
						<div class="aititle">实时安全预警
            <el-button type="primary" @click="plupdate">批量销警</el-button>
            <el-button :class="yxj == false ? 'xjbutton' : ''" @click="qiehuan('0')">未销警</el-button>
            <el-button :class="yxj == true ? 'xjbutton' : ''" @click="qiehuan('1')">已销警</el-button>
                ...
                <span v-if="queryParams.ifcl == '1'">
									<img src="@/assets/images/safework/ai-ico5.png" />
									处理结果：{{bjxx.cljg}}
								</span>
                <span v-if="queryParams.ifcl == '1'">
									<img src="@/assets/images/safework/ai-ico4.png" />
									处理时间：{{bjxx.updateTime}}
								</span>
							</div>
              <el-button v-if="queryParams.ifcl == '0'" @click="xiaojing(bjxx.id)" class="xiaojing">消警</el-button>
						</div>
					</div>
				</el-col>
			</el-row>
...
<script>
import { listBjxx,updateBjxx,plupdateHkBjxx } from "@/api/aiWarning/bjxx";
import { listZjxx } from "@/api/aiWarning/zjxx";
...
export default {
  ...
  methods: {
    ...
    getzjxx() {
      listZjxx({zjStatus:1}).then(res => {
        this.zjxx = res.rows[0]
      })
    },
    ...
    xiaojing(id) {
      this.$prompt('销警备注', '提示', {
        ...
        updateBjxx({id:id,ifcl:'1',cljg:value}).then(res => {
          this.getList()
        })
    ...
    plupdate(){
      this.$prompt('批量销警备注', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
      }).then(({ value }) => {
        plupdateHkBjxx({ifcl:'1',cljg:value}).then(res => {
          this.getList()
        })
    ...
    getList() {
      listBjxx(this.queryParams).then(response => {
