...
<script setup>
import { listIndustrySensor,addIndustryAreaSensors } from "@/api/industry/industryAreaSensor";
...
/** 查询表数据 */
function getList() {
  listIndustrySensor(queryParams.value).then(res => {
...
function handleSelectDevice() {
  ...
  if (sensorIds == "") {
    ...
    return;
		...
		form.value.industryAreaId = props.industryAreaId;
		form.value.sensorIds = sensorIds;
		addIndustryAreaSensors(form.value).then(response => {
			proxy.$modal.msgSuccess("新增成功");
			if (response.code === 200) {
				visible.value = false;
				emit("ok",props.industryAreaId);
			}
		});
	}
}
