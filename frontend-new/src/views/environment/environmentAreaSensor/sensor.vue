<template>
  ...
  <el-dialog title="选择设备" v-model="visible" width="800px" top="5vh" append-to-body>
    <el-row>
      ...
      <el-table :data="sensorList" height="260px"  @row-click="clickRow" ref="table" @selection-change="handleSelectionChange">
    ...
    <template #footer>
      <div class="dialog-footer">
        <el-button type="primary" @click="handleSelectDevice" auto-insert-spacev-hasPermi="['environment:environment-sensor:add']">确 定</el-button>
...
<script setup>
import { listEnvironmentSensor,addEnvironmentAreaSensors } from "@/api/environment/environmentAreaSensor";
...
/** 查询表数据 */
function getList() {
  listEnvironmentSensor(queryParams.value).then(res => {
...
function handleSelectDevice() {
  ...
  if (sensorIds == "") {
    ...
    return;
		...
		form.value.environmentAreaId = props.environmentAreaId;
		form.value.sensorIds = sensorIds;
		addEnvironmentAreaSensors(form.value).then(response => {
			proxy.$modal.msgSuccess("新增成功");
			if (response.code === 200) {
				visible.value = false;
				emit("ok",props.environmentAreaId);
			}
		});
	}
}
