    ...
    <div class="dianjianjilu">
      <div class="topform">
        <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
          <el-row>
            ...
            <el-col :span="7">
              <el-form-item label="点检月份" prop="taskMonth">
      ...
      <div class="botbox">
        <el-card class="box-card" shadow="never">
          <template #header>
            <div class="card-header">点检记录</div>
          </template>
          <el-table border stripe height="calc(100% - 50px)" v-loading="loading" :data="spotCheckMonthList" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="点检月份" align="center" prop="taskMonth" width="180">
              <template #default="scope">
                <span>{{ parseTime(scope.row.taskMonth, '{y}-{m}') }}月</span>
              </template>
            </el-table-column>
            <el-table-column label="项目数量" align="center" prop="num1" />
            <el-table-column label="异常数量" align="center" prop="numError" />
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
              <template #default="scope">
                <el-button class="detbtn"
                           type="text"
                           @click="handleDetail(scope.row)"
                           v-hasPermi="['equipment:spotCheck:list']"
                >详情</el-button>
              </template>
            </el-table-column>
          </el-table>
...
function getList() {
  ...
  loading.value = true;
  listSpotCheckCount(queryParams.value).then(response => {
