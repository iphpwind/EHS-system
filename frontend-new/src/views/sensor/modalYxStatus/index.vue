<template>
  <div class="app-container">
    ...
    <el-table height="calc(100vh - 300px)" v-loading="loading" :data="SensorModalYxStatusList"
      ...
      <el-table-column label="模板id" align="center" prop="yxstatusModalcode"/>
      ...
      <el-table-column label="遥测状态设定值对应关系" align="center" prop="ycStatusSetValueRelation"
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="SensorModalYxStatusRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="模板id" prop="yxstatusModalcode">
          <el-input v-model="form.yxstatusModalcode" placeholder="请输入模板id" readonly/>
        </el-form-item>
        <el-form-item label="遥信序列号" prop="yxSerial">
          <el-input v-model="form.yxSerial" placeholder="请输入遥信序列号"/>
        </el-form-item>
        <el-form-item label="遥信状态设定值" prop="yxstatusSetvalue">
...
<script setup name="modalYxStatus">
import {
  listSensorModalYxStatus,
  getSensorModalYxStatus,
  delSensorModalYxStatus,
  addSensorModalYxStatus,
  updateSensorModalYxStatus
} from "@/api/sensor/modalYxStatus";
import {h} from "vue";

const {proxy} = getCurrentInstance();
const {work_status} = proxy.useDict('work_status');

const SensorModalYxStatusList = ref([]);
...
const data = reactive({
  ...
  queryParams: {
    pageNum: 1,
    pageSize: 10,
    yxstatusModalcode: route.params && route.params.modalCode
  },
  rules: {},
...
function getList() {
  loading.value = true;
  listSensorModalYxStatus(queryParams.value).then(response => {
    SensorModalYxStatusList.value = response.rows;
...
function reset() {
  ...
  proxy.resetForm("SensorModalYxStatusRef");
}
...
function handleUpdate(row) {
  ...
  getSensorModalYxStatus(yxstatusNo).then(response => {
    form.value = response.data;
    open.value = true;
    title.value = "修改运行状态模型";
  });
}

/** 提交按钮 */
function submitForm() {
  proxy.$refs["SensorModalYxStatusRef"].validate(valid => {
    if (valid) {
      if (form.value.yxstatusNo != null) {
        updateSensorModalYxStatus(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addSensorModalYxStatus(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm(h('p',null,[h('span', { style: 'color: black' },'确定删除该数据吗'),h('p', { style: 'color: red' }, "删除后不可恢复!!!")]),{
    ...
    return delSensorModalYxStatus(yxstatusNos);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('sensor/SensorModalYxStatus/export', {
    ...queryParams.value
  }, `SensorModalYxStatus_${new Date().getTime()}.xlsx`)
}
