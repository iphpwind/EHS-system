    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['safework:visitDic:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['safework:visitDic:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['safework:visitDic:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['safework:visitDic:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="visitDicList" @selection-change="handleSelectionChange">
...
<!--      <el-table-column label="企业编码" align="center" prop="enterpriseCode" />-->
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="visitDicRef" :model="form" :rules="rules" label-width="80px">
<!--        <el-form-item label="企业编码" prop="enterpriseCode">-->
...
function handleUpdate(row) {
  ...
  getVisitDic(id).then(response => {
    form.value = response.data;
    open.value = true;
    title.value = "修改访问事由字典";
  });
}

/** 提交按钮 */
function submitForm() {
  proxy.$refs["visitDicRef"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateVisitDic(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addVisitDic(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除访问事由字典编号为"' + idss + '"的数据项？').then(function() {
    return delVisitDic(idss);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('safework/visitDic/export', {
    ...queryParams.value
  }, `visitDic_${new Date().getTime()}.xlsx`)
}
