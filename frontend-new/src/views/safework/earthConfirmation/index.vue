    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['safework:earthConfirmation:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['safework:earthConfirmation:edit']"
        >修改</el-button>
      </el-col>
<!--      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['safework:earthConfirmation:remove']"
        >删除</el-button>
      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['safework:earthConfirmation:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="earthConfirmationList" @selection-change="handleSelectionChange">
...
<!--          <el-button
    ...
    <el-dialog :title="title" v-model="open" width="600px" append-to-body>
      <el-form ref="earthConfirmationRef" :model="form" :rules="rules" label-width="120px">
        <el-form-item label="现场确认部门" prop="confirmationDeptDicId">
...
<script setup name="EarthConfirmation">
...
const earthConfirmationList = ref([]);
...
function getSceneConfirmationDeptDicList() {
  listSceneConfirmationDeptDic({status:"0", operationType:"1"}).then(response => {
...
function getList() {
  loading.value = true;
  listEarthConfirmation(queryParams.value).then(response => {
    earthConfirmationList.value = response.rows;
...
function reset() {
  ...
  proxy.resetForm("earthConfirmationRef");
}
...
function handleUpdate(row) {
  ...
  getEarthConfirmation(id).then(response => {
    form.value = response.data;
    open.value = true;
    title.value = "修改动土作业现场确认内容";
  });
}

/** 提交按钮 */
function submitForm() {
  proxy.$refs["earthConfirmationRef"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateEarthConfirmation(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addEarthConfirmation(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm(h('p',null,[h('span', { style: 'color: black' },'确定删除该数据吗'),h('p', { style: 'color: red' }, "删除后不可恢复!!!")]),{
    ...
    return delEarthConfirmation(idss);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('safework/earthConfirmation/export', {
    ...queryParams.value
  }, `earthConfirmation_${new Date().getTime()}.xlsx`)
}
