  ...
  <div class="beijian">
      ...
      <div class="seachform">
        <el-form :inline="true" :model="queryParams" >
          <el-form-item label="分析对象">
            <el-select v-model="queryParams.id" clearable filterable placeholder="全部">
              <el-option
          ...
          <el-form-item>
            <el-button type="primary" @click="goNoPlan" class="checkbtn">未列计划排查内容</el-button>
          </el-form-item>
        </el-form>
      </div>
    </el-card>
    ...
    <div class="table">
      <el-table
          ...
          v-loading="loading"
      >
        <el-table-column prop="name"  label="分析对象"/>
        <el-table-column prop="unitCount"  label="单元"/>
        <el-table-column prop="eventCount"  label="风险事件" />
        <el-table-column prop="measureCount"  label="管控措施" />
        <el-table-column prop="measureCount"  label="排查内容" />
...
<script>
import { listObject,selectHuizong } from "@/api/safework/object";
...
export default {
  ...
  methods: {
    ...
    getList() {
      selectHuizong(this.queryParams).then(res => {
    ...
    goNoPlan() {
      this.$router.push({
        path:'/safework/riskNotPlan',
      })
    },
    goDetail(row) {
      this.$router.push({
        path:'/safework/riskDetail',
        query:{
          obId:row.id
        }
      })
    ...
    handleDown(row) {
      this.download('safework/measure/export', {
        objectId:row.id
