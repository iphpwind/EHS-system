    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['safework:brokenSceneConfirmation:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['safework:brokenSceneConfirmation:edit']"
        >修改</el-button>
      </el-col>
<!--      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['safework:brokenSceneConfirmation:remove']"
        >删除</el-button>
      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['safework:brokenSceneConfirmation:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="brokenSceneConfirmationList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="来源" align="center" prop="originType">
        <template #default="scope">
          <el-row justify="center" v-if="scope.row.originType == 0">国标</el-row>
          <el-row justify="center" v-else>自定义</el-row>
        </template>
      </el-table-column>
...
<!--          <el-button
    ...
    <!-- 添加或修改断路作业现场确认内容字典对话框 -->
    <el-dialog :title="title" v-model="open" width="600px" append-to-body>
      <el-form ref="brokenSceneConfirmationRef" :model="form" :rules="rules" label-width="120px">
        <el-form-item label="现场确认部门" prop="confirmationDeptDicId">
        ...
        <el-form-item label="现场确认序号" prop="orderNum">
          <el-input v-model="form.orderNum" placeholder="请输入现场确认序号" />
        </el-form-item>
        <el-form-item label="现场确认内容" prop="confirmationContent">
          <el-input v-model="form.confirmationContent" placeholder="请输入现场确认内容" />
        </el-form-item>
...
<script setup name="BrokenSceneConfirmation">
...
const brokenSceneConfirmationList = ref([]);
...
function getSceneConfirmationDeptDicList() {
  listSceneConfirmationDeptDic({status:"0", operationType:"3"}).then(response => {
...
function getList() {
  loading.value = true;
  listBrokenSceneConfirmation(queryParams.value).then(response => {
    brokenSceneConfirmationList.value = response.rows;
...
function reset() {
  ...
  proxy.resetForm("brokenSceneConfirmationRef");
}
...
function handleUpdate(row) {
  ...
  getBrokenSceneConfirmation(id).then(response => {
    form.value = response.data;
    open.value = true;
    title.value = "修改断路作业现场确认内容字典";
  });
}

/** 提交按钮 */
function submitForm() {
  proxy.$refs["brokenSceneConfirmationRef"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateBrokenSceneConfirmation(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addBrokenSceneConfirmation(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm(h('p',null,[h('span', { style: 'color: black' },'确定删除该数据吗'),h('p', { style: 'color: red' }, "删除后不可恢复!!!")]),{
    ...
    return delBrokenSceneConfirmation(idss);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('safework/brokenSceneConfirmation/export', {
    ...queryParams.value
  }, `brokenSceneConfirmation_${new Date().getTime()}.xlsx`)
}
