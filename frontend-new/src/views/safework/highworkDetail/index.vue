<template>
  <div class="report">
    <div class="title"><span>高处作业作业详情</span>
    ...
    <el-collapse @change="handleChange">
      <el-collapse-item title="基础信息" name="1">
        ...
        <div class="earthitem">
          <h4>现场确认</h4>
          <table>
            <tr v-for="(item,index) in form.safeOperationHighworkSceneUserList">
        ...
        <div class="earthitem">
          <h4>作业审批</h4>
          <table>
            <tr v-for="item in form.safeOperationHighworkApprovalList">
        ...
        <div class="earthitem">
          <h4>作业验收</h4>
          <table>
            <tr v-for="(item,index) in form.safeOperationHighworkCheckList">
      ...
      <el-collapse-item title="现场确认" name="2">
        <div v-for="item in form.safeOperationHighworkSceneUserList" class="earthitem">
          <h4>{{item.departmentName}}</h4>
          <div v-for="aa in item.safeOperationHighworkSceneContentList" >
      ...
      <el-collapse-item title="作业审核" name="3">
        <div class="earthitem" v-for="item in form.safeOperationHighworkApprovalList">
          <table>
            <tr>
              <td>{{item.operationName}}：</td>
      ...
      <el-collapse-item title="作业验收" name="4">
        <div class="earthitem" v-for="item in form.safeOperationHighworkCheckList">
          <table>
            <tr>
              <td>{{item.checkName}}</td>
              <td colspan="2"></td>
...
export default {
  ...
  methods: {
    ...
    getRefuse(){
      listRefuse({operationType:'4',type:'0',mainId:this.id}).then(response => {
        if(response.rows.length>0){
          this.cancelform = response.rows[0];
        }
      })
      listRefuse({operationType:'4',type:'1',mainId:this.id}).then(response => {
