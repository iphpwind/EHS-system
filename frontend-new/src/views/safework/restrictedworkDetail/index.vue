<template>
  <div class="report">
    <div class="title"><span>受限空间作业详情</span>
    ...
    <el-collapse @change="handleChange">
      <el-collapse-item title="基础信息" name="1">
        <div class="earthitem">
          ...
          <table>
            ...
            <tr>
              <td>风险辨识结果：</td>
              <td colspan="2">{{ form.hazardName }}</td>
            </tr>
            <tr>
              <td>作业危害分析人：</td>
            ...
            <tr>
              <td>作业人：</td>
              <td colspan="2">
                <span v-for="(item,index) in form.safeOperationRestrictedUserList">
                  <img :src="item.sign" alt="" class="qianming">
                </span>
              </td>
            </tr>
        ...
        <div class="earthitem">
          <h4>现场确认</h4>
          <table>
            <tr v-for="(item,index) in form.safeOperationRestrictedworkSceneUserList">
        ...
        <div class="earthitem">
          <h4>作业审批</h4>
          <table>
            <tr v-for="item in form.safeOperationRestrictedworkApprovalList">
        ...
        <div class="earthitem">
          <h4>作业验收</h4>
          <table>
            <tr v-for="(item,index) in form.safeOperationRestrictedworkCheckList">
      ...
      <el-collapse-item title="分析化验" name="2">
        <div v-for="item in form.safeOperationRestrictedworkAnalysisList" class="earthitem">
          <table>
            ...
            <tr>
              <td>分析人：</td>
              <td colspan="2"><img :src="item.analysisUserSign" alt="" class="qianming"></td>
            </tr>
            <tr>
              <td>分析点位：</td>
              <td colspan="2">{{ item.analysisSiteName }}</td>
            </tr>
            <tr>
              <td>提交人：</td>
            ...
            <tr>
              <td>现场照片：</td>
              <td colspan="2">
                <img :src="item.scenePic" alt="" class="qianming">
              </td>
            </tr>
          </table>
        </div>
      ...
      <el-collapse-item title="现场确认" name="3">
        <div v-for="item in form.safeOperationRestrictedworkSceneUserList" class="earthitem">
          <h4>{{item.departmentName}}</h4>
          <div v-for="aa in item.safeOperationRestrictedworkSceneContentList" >
            <el-checkbox class="checkbox" v-model="aa.checkFlag" disabled>{{aa.confirmationContent}}</el-checkbox>
          </div>
          <table>
            <tr>
              <td>其他安全措施：</td>
      ...
      <el-collapse-item title="作业审核" name="4">
        <div class="earthitem" v-for="item in form.safeOperationRestrictedworkApprovalList">
          <table>
            <tr>
              <td>{{item.operationName}}：</td>
      ...
      <el-collapse-item title="作业验收" name="5">
        <div class="earthitem" v-for="item in form.safeOperationRestrictedworkCheckList">
          <table>
            <tr>
              <td>{{item.checkName}}</td>
              <td colspan="2"></td>
...
export default {
  ...
  methods: {
    ...
    getRefuse(){
      listRefuse({operationType:'7',type:'0',mainId:this.id}).then(response => {
        if(response.rows.length>0){
          this.cancelform = response.rows[0];
        }
      })
      listRefuse({operationType:'7',type:'1',mainId:this.id}).then(response => {
