<template>
  <div class="app-container">
    ...
    <el-table height="calc(100vh - 300px)" v-loading="loading" :data="hoistingworkList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="所属机构" align="center" prop="deptName" />
      <el-table-column label="作业票编码" align="center" prop="code" />
      ...
      <el-table-column label="吊装作业级别" align="center" prop="levelName" />
      <el-table-column label="吊装作业吊具" align="center" prop="typeName" />
      <!-- <el-table-column label="吊装作业计划开始时间" align="center" prop="startTime" width="180">
        <template #default="scope">
          <span>{{ parseTime(scope.row.startTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="吊装作业计划结束时间" align="center" prop="endTime" width="180">
        <template #default="scope">
...
<script setup name="Hoistingwork">
import { listHoistingwork, getHoistingwork, delHoistingwork, addHoistingwork, updateHoistingwork, updateStatus } from "@/api/safework/hoistingwork";
...
function getList() {
  loading.value = true;
  listHoistingwork(queryParams.value).then(response => {
    hoistingworkList.value = response.rows;
...
function ticket(){
  proxy.$router.push({
    path: '/SpecialOperation/hoistingworkTicketdemo',
  })
}
...
function lookDetail(row){
  proxy.$router.push({
    path: '/SpecialOperation/hoistingworkTicket',
    query:{
      id:row.id
    }
  })
...
function detail(row){
  proxy.$router.push({
    path: '/SpecialOperation/hoistingworkDetail',
    query:{
      id:row.id
    }
  })
...
function reset() {
  ...
  proxy.resetForm("hoistingworkRef");
}
