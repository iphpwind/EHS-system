<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['safework:chemicalFinalProduction:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['safework:chemicalFinalProduction:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['safework:chemicalFinalProduction:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['safework:chemicalFinalProduction:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="chemicalFinalProductionList" @selection-change="handleSelectionChange">
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="chemicalFinalProductionRef" :model="form" :rules="rules" label-width="170px">
...
<script setup name="ChemicalFinalProduction">
import { listChemicalFinalProduction, getChemicalFinalProduction, delChemicalFinalProduction, addChemicalFinalProduction, updateChemicalFinalProduction } from "@/api/safework/chemicalFinalProduction";
import {h} from "vue";

const { proxy } = getCurrentInstance();
const { status, chemical_physical_form } = proxy.useDict('status', 'chemical_physical_form');

const chemicalFinalProductionList = ref([]);
...
const data = reactive({
  ...
  rules: {
    ...
    annualOutput: [
      { required: true, message: "年产量不能为空", trigger: "blur" }
    ],
    maximumStorage: [
...
function getList() {
  loading.value = true;
  listChemicalFinalProduction(queryParams.value).then(response => {
    chemicalFinalProductionList.value = response.rows;
...
function reset() {
  ...
  proxy.resetForm("chemicalFinalProductionRef");
}
...
function handleUpdate(row) {
  ...
  getChemicalFinalProduction(id).then(response => {
    form.value = response.data;
    open.value = true;
    title.value = "修改最终产品管理";
  });
}

/** 提交按钮 */
function submitForm() {
  proxy.$refs["chemicalFinalProductionRef"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateChemicalFinalProduction(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addChemicalFinalProduction(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm(h('p',null,[h('span', { style: 'color: black' },'确定删除该数据吗'),h('p', { style: 'color: red' }, "删除后不可恢复!!!")]),{
    ...
    return delChemicalFinalProduction(idss);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('safework/chemicalFinalProduction/export', {
    ...queryParams.value
  }, `chemicalFinalProduction_${new Date().getTime()}.xlsx`)
}
