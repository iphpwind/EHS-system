<template>
  <div class="app-container">
    ...
    <el-table height="calc(100vh - 300px)" v-loading="loading" :data="restrictedworkList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="所属机构" align="center" prop="deptName" />
      <el-table-column label="作业证编码" align="center" prop="id" />
      ...
      <el-table-column label="受限空间内原有介质" align="center" prop="mediumName" />
      <!-- <el-table-column label="受限空间计划开始时间" align="center" prop="startTime" width="180">
        <template #default="scope">
          <span>{{ parseTime(scope.row.startTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="受限空间计划结束时间" align="center" prop="endTime" width="180">
        <template #default="scope">
...
<script setup name="Restrictedwork">
import { listRestrictedwork, getRestrictedwork, delRestrictedwork, addRestrictedwork, updateRestrictedwork,updateStatus } from "@/api/safework/restrictedwork";
...
function getList() {
  loading.value = true;
  listRestrictedwork(queryParams.value).then(response => {
    restrictedworkList.value = response.rows;
...
function ticket(){
  proxy.$router.push({
    path: '/SpecialOperation/restrictedworkTicketdemo',
  })
}
...
function lookDetail(row){
  proxy.$router.push({
    path: '/SpecialOperation/restrictedworkTicket',
    query:{
      id:row.id
    }
  })
...
function detail(row){
  proxy.$router.push({
    path: '/SpecialOperation/restrictedworkDetail',
    query:{
      id:row.id
    }
  })
...
function reset() {
  ...
  proxy.resetForm("restrictedworkRef");
}
