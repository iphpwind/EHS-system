<template>
  <div class="app-container">
    ...
    <el-table height="calc(100vh - 300px)" v-loading="loading" :data="highworkList" @selection-change="handleSelectionChange">
      ...
      <!-- <el-table-column label="高处作业计划开始时间" align="center" prop="startTime" width="180">
        <template #default="scope">
          <span>{{ parseTime(scope.row.startTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="高处作业计划结束时间" align="center" prop="endTime" width="180">
        <template #default="scope">
...
<script setup name="Highwork">
import { listHighwork, getHighwork, delHighwork, addHighwork, updateHighwork, updateStatus } from "@/api/safework/highwork";
...
function getList() {
  loading.value = true;
  listHighwork(queryParams.value).then(response => {
    highworkList.value = response.rows;
...
function ticket(){
  proxy.$router.push({
    path: '/SpecialOperation/highworkTicketdemo',
  })
}
...
function lookDetail(row){
  proxy.$router.push({
    path: '/SpecialOperation/highworkTicket',
    query:{
      id:row.id
    }
  })
...
function detail(row){
  proxy.$router.push({
    path: '/SpecialOperation/highworkDetail',
    query:{
      id:row.id
    }
  })
