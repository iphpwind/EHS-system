<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="100px">
      ...
      <el-form-item label="隐患主管部门" prop="deptids">
        <el-select v-model="queryParams.deptinfos" placeholder="请选择" clearable filterable multiple>
          <el-option
      ...
      <el-form-item label="隐患来源" prop="troubleSourceId">
        <el-select v-model="queryParams.troubleSourceId" placeholder="请选择" clearable filterable>
          <el-option
              v-for="item in resourcecolumns"
              :key="item.id"
              :label="item.troubleSourceName"
              :value="item.id"
          />
        </el-select>
      </el-form-item>
      <el-form-item label="隐患类型" prop="troubleTypeId">
        <el-select v-model="queryParams.troubleTypeId" placeholder="请选择" clearable filterable>
          <el-option
              v-for="item in catagcolumns"
              :key="item.id"
              :label="item.troubleTypeName"
              :value="item.id"
          />
        </el-select>
      </el-form-item>
      <el-form-item label="上报人" prop="reportUser">
        <el-select  v-model="queryParams.reportUser" clearable>
          <el-option
              v-for="user in users"
              :key="user.userId"
              :label="user.nickName"
      ...
      <el-form-item label="整改负责人" prop="zgChargeUserName">
        <el-input
            v-model="queryParams.zgChargeUserName"
            placeholder="请输入整改负责人"
            clearable
            @keyup.enter="handleQuery"
        />
      </el-form-item>
      <el-form-item label="是否超期" prop="cqwzg">
        <el-select  v-model="queryParams.cqwzg" clearable>
          <el-option label="超期未整改" value="cqwzg" />
        </el-select>
      </el-form-item>
      <el-form-item label="状态" prop="status">
        <el-select  v-model="queryParams.status" clearable>
          ...
          <el-option label="整改中" value="3" />
          <el-option label="分管验收" value="4" />
          <el-option label="主管验收" value="5" />
      ...
      <el-form-item>
        ...
        <el-button type="primary" icon="Download" @click="handExport">导出模板</el-button>
        <el-button type="primary" icon="upload" @click="handleAdd">批量上报</el-button>
    ...
    <el-table height="calc(100vh - 440px)" v-loading="loading" :data="hazareportList" @selection-change="handleSelectionChange">
    ...
    <el-dialog title="详情" v-model="detailOpen" width="500px" append-to-body>
      <div class="report">
        ...
        <el-collapse @change="handleChange">
          <el-collapse-item title="上报" name="1" v-if="sbform!=null && sbform!=''">
            <div class="earthitem">
              <h4>上报</h4>
              <table>
                <tr>
          ...
          <el-collapse-item title="评估" name="2" v-if="pgform!=null && pgform!=''">
            <div class="earthitem">
              <h4>评估</h4>
              <table>
                <tr>
                  <td>隐患等级：</td>
                  <td colspan="2">{{ pgform.haLevel }}</td>
                </tr>
                <tr>
                  <td>不合格因素分类：</td>
                  <td colspan="2">{{ pgform.nonFactorsClassName }}</td>
                </tr>
                <tr>
                  <td>不合格因素描述：</td>
                  <td colspan="2">{{ pgform.nonFactors }}</td>
                </tr>
                <tr>
                  <td>管控层级：</td>
                  <td colspan="2">{{ pgform.levelName }}</td>
                </tr>
                <tr>
                  <td>整改负责人：</td>
                  <td colspan="2">{{ sbform.zgChargeUserName }}</td>
                </tr>
              </table>
            </div>
          </el-collapse-item>
          <el-collapse-item title="待整改" name="3" v-if="dzgform!=null && dzgform!=''">
            <h4>待整改</h4>
            <table>
              <tr>
                <td>整改意见：</td>
          ...
          <el-collapse-item title="整改中" name="4" v-if="zgform!=null && zgform!=''">
            <div class="earthitem">
              <h4>整改中</h4>
              <table>
                <tr>
          ...
          <el-collapse-item title="验收" name="5" v-if="ysform!=null && ysform!=''">
            <div class="earthitem">
              <h4>验收</h4>
              <table>
                <tr>
...
import { listHazareport, getHazareport } from "@/api/safework/hazareport";
import {getRecord} from "@/api/safework/record";
import { listHazardattachment} from "@/api/safework/hazardattachment";
import { listNotrectified } from "@/api/safework/notrectified";
import { listAssessment} from "@/api/safework/assessment";
import { listRectification } from "@/api/safework/rectification";
import { listRectmeasure } from "@/api/safework/rectmeasure";
import { listAccept} from "@/api/safework/accept";
import {userList} from "@/api/system/user";
import { listDept } from "@/api/system/dept";
import { listTroublesource} from "@/api/safework/troublesource";
import { listInvestigate } from "@/api/safework/investigate";
import { listTroubletype } from "@/api/safework/troubletype";
...
const catagcolumns = ref([]);
const single = ref(true);
const multiple = ref(true);
const total = ref(0);
const title = ref("异常详情");
const investigates = ref([]);
const upload = reactive({
  ...
  // 上传的地址
  url: import.meta.env.VITE_APP_BASE_API + "/safework/hazareport/importData"
});
...
function getList() {
  loading.value = true;
  queryParams.value.hazardFlag = '限时整改'
  if(queryParams.value.deptinfos!=null && queryParams.value.deptinfos!=''){
    queryParams.value.deptids = queryParams.value.deptinfos.join(",");
  }else{
...
function getCatag() {
  listTroubletype({delFlag: 0}).then(response => {
    catagcolumns.value = response.rows;
  });
}

function getTrouble(){
  listTroublesource({delFlag:0}).then(response => {
    resourcecolumns.value = response.rows;
  });
}
...
/** 导出模板操作 */
function handExport() {
  proxy.download('safework/hazareport/exportModel', {
    ...queryParams.value
  }, `hazareport_${new Date().getTime()}.xlsx`)
}
