<template>
  <div class="report">
    <div class="title"><span>动火作业详情</span>
    ...
    <el-collapse @change="handleChange">
      <el-collapse-item title="基础信息" name="1">
        <div class="earthitem">
          ...
          <table>
            ...
            <tr>
              <td>作业危害分析人：</td>
              <td colspan="2"><img :src="form.hazardAnalysisSign" alt="" class="qianming"></td>
            </tr>
            <tr>
              <td>作业危害审核人：</td>
              <td colspan="2"><img :src="form.hazardProcessSign" alt="" class="qianming"></td>
            </tr>
            <tr>
              <td>作业危害审定人：</td>
              <td colspan="2"><img :src="form.hazardApprovalSign" alt="" class="qianming"></td>
            </tr>
            <tr>
              <td>作业单位：</td>
            ...
            <tr>
              <td>动火人：</td>
              <td colspan="2">
                <span v-for="(item,index) in form.safeOperationFireworkFireuserList">
                  <img :src="item.sign" alt="" class="qianming">
                </span>
              </td>
            </tr>
        ...
        <div class="earthitem">
          ...
          <table>
            <tr>
              <td>分析数据：</td>
              <td colspan="2">{{ form.analysisContent }}</td>
            </tr>
            <tr>
              <td>分析人：</td>
              <td colspan="2">{{ form.analysisUserName }}</td>
            </tr>
          </table>
        </div>
        <div class="earthitem">
          <h4>现场确认</h4>
          <table>
            <tr v-for="(item,index) in form.safeOperationFireworkSceneUserList">
        ...
        <div class="earthitem">
          <h4>作业审批</h4>
          <table>
            <tr v-for="item in form.safeOperationFireworkApprovalList">
        ...
        <div class="earthitem">
          <h4>作业验收</h4>
          <table>
            <tr v-for="(item,index) in form.safeOperationFireworkCheckList">
      ...
      <el-collapse-item title="分析化验" name="2">
        <div v-for="item in form.safeOperationFireworkAnalysisList" class="earthitem">
          <table>
            <tr class="fxhxtbtopth">
              <th width="28%">分析数据名称</th>
              <th width="12%">单位</th>
              <th width="18%">正常值范围</th>
              <th width="12%">结果</th>
            </tr>
            <tr v-for="(aa, index) in item.analysisList" :class="'fxhxtbtop' + index">
              <td>{{aa.analysisName}}</td>
              <td>{{aa.numUnit}}</td>
              <td v-if="aa.analysisType == '0'">{{aa.numLow}}-{{aa.numUp}}</td>
              <td v-if="aa.analysisType == '1'">{{aa.stringArea}}</td>
              <td>
                {{aa.analysisResult}}
              </td>
            </tr>
            <tr>
              <td>分析人：</td>
              <td colspan="3"><img :src="item.analysisUserSign" alt="" class="qianming"></td>
            </tr>
            <tr>
              <td>分析点位：</td>
              <td colspan="3">{{ item.analysisSiteName }}</td>
            </tr>
            <tr>
              <td>提交人：</td>
              <td colspan="3">{{ item.createBy }}</td>
            </tr>
            <tr>
              <td>提交时间：</td>
            ...
            <tr>
              <td>现场照片：</td>
              <td colspan="3">
                <div style="float: left;margin-left: 10px" v-for="(aa,index) in item.AnalysisPicList">
      ...
      <el-collapse-item title="现场确认" name="3">
        <div v-for="item in form.safeOperationFireworkSceneUserList" class="earthitem">
          <h4>{{item.departmentName}}</h4>
          <div v-for="aa in item.safeOperationFireworkSceneContentList" >
            <el-checkbox class="checkbox" v-model="aa.checkFlag" disabled>{{aa.confirmContent}}</el-checkbox>
          </div>
          <table>
            <tr>
              <td>其他安全措施：</td>
            ...
            <tr>
              <td>现场照片：</td>
              <td colspan="2">
                <div style="float: left;margin-left: 10px" v-for="(aa,index) in item.scenePicList">
                  <img :src="aa" alt="" class="qianming">
                </div>
              </td>
            </tr>
      ...
      <el-collapse-item title="作业审核" name="4">
        <div class="earthitem" v-for="item in form.safeOperationFireworkApprovalList">
          <table>
            <tr>
              <td>{{item.operationName}}：</td>
      ...
      <el-collapse-item title="作业验收" name="5">
        <div class="earthitem" v-for="item in form.safeOperationFireworkCheckList">
          <table>
            <tr>
              <td>{{item.checkName}}</td>
              <td colspan="2"></td>
  ...
  methods: {
    getInfo(){
      getFirework(this.id).then(response => {
        ...
        this.form.safeOperationFireworkAnalysisList.forEach((j, index) => {
          if (j.scenePic != null){
            j.AnalysisPicList = j.scenePic.split(',')
          }else {
            j.AnalysisPicList = []
          }
        })
        ...
        this.form.safeOperationFireworkSceneUserList.forEach((i, index) => {
          if (i.scenePics != null){
            i.scenePicList = i.scenePics.split(',')
          }else {
            i.scenePicList = []
          }
        })
    ...
    getRefuse(){
      listRefuse({operationType:'0',type:'0',mainId:this.id}).then(response => {
        if(response.rows.length>0){
          this.cancelform = response.rows[0];
        }
      })
      listRefuse({operationType:'0',type:'1',mainId:this.id}).then(response => {
...
:deep(.earthitem .checkbox .el-checkbox__label) {
  white-space: initial;
  color: #303133;
}
