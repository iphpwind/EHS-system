<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['safework:chemicalProductionMaterial:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['safework:chemicalProductionMaterial:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['safework:chemicalProductionMaterial:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['safework:chemicalProductionMaterial:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="chemicalProductionMaterialList" @selection-change="handleSelectionChange">
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="chemicalProductionMaterialRef" :model="form" :rules="rules" label-width="170px">
...
<script setup name="ChemicalProductionMaterial">
import { listChemicalProductionMaterial, getChemicalProductionMaterial, delChemicalProductionMaterial, addChemicalProductionMaterial, updateChemicalProductionMaterial } from "@/api/safework/chemicalProductionMaterial";
import {h} from "vue";

const { proxy } = getCurrentInstance();
const { status, chemical_physical_form } = proxy.useDict('status', 'chemical_physical_form');

const chemicalProductionMaterialList = ref([]);
...
function getList() {
  loading.value = true;
  listChemicalProductionMaterial(queryParams.value).then(response => {
    chemicalProductionMaterialList.value = response.rows;
...
function reset() {
  ...
  proxy.resetForm("chemicalProductionMaterialRef");
}
...
function handleUpdate(row) {
  ...
  getChemicalProductionMaterial(id).then(response => {
    form.value = response.data;
    open.value = true;
    title.value = "修改生产原料管理";
  });
}

/** 提交按钮 */
function submitForm() {
  proxy.$refs["chemicalProductionMaterialRef"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateChemicalProductionMaterial(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addChemicalProductionMaterial(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm(h('p',null,[h('span', { style: 'color: black' },'确定删除该数据吗'),h('p', { style: 'color: red' }, "删除后不可恢复!!!")]),{
    ...
    return delChemicalProductionMaterial(idss);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('safework/chemicalProductionMaterial/export', {
    ...queryParams.value
  }, `chemicalProductionMaterial_${new Date().getTime()}.xlsx`)
}
